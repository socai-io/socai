    ...
    pub fn execution_prompt(&self) -> anyhow::Result<String> {
        ...
        Ok(format!(
            "## Coverage-guided research execution\n\n\
             ...
             - In submit_research_completion, report every brief subquestion exactly once and prefer the system-generated E1/E2 evidence IDs; do not invent or reconstruct locator paths.\n\
             - Set gap_kind=none for covered subquestions. Classify other gaps as: material when an ordinary available-tool action may change the core answer; search_scarcity when one precise last-mile search may find a higher-match example; capability_blocked when current tools cannot enter the required source; source_unavailable when the fact is not public; limitation when it does not change the core answer. A failed keyword search, captcha, login wall, rate limit, or browser failure is never a coverage-blocked gap.\n\
             - Before classifying every top-level gap, answer the blocked challenge fields literally: is the exact required source available_with_current_tools; is there one concrete non-duplicate candidate_action; and was that action already attempted. XHS reposts or screenshots do not make a government website directly reachable. If the required source is unreachable, use capability_blocked/source_unavailable rather than limitation. If an available untried action remains, use material/search_scarcity rather than limitation.\n\
             - Despite its legacy name, material_gap must describe every non-covered subquestion. Material and search_scarcity need a precise next_action. Capability-blocked/source-unavailable top-level gaps need auditable source, attempted-action, impact, unblock, and hard-constraint fields. If such a gap blocks a hard constraint, hard_constraints_satisfied must be false.\n\
