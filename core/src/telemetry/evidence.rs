...
fn archive_stage_lock() -> &'static Mutex<()> {
...
pub(crate) fn stage_run_archive(
    ...
    // Cancellation, task deletion, and the normal completion path may all try
    // to stage the same run. Once a pending archive exists, keep those bytes
    // immutable so an upload checkpoint always refers to the same batches.
    let _guard = archive_stage_lock()
        .lock()
        .map_err(|_| io::Error::other("evidence archive staging lock is poisoned"))?;
    let archive = if destination.is_file() || local_path.is_file() {
        let source = if destination.is_file() {
            &destination
        } else {
            &local_path
        };
        let existing = read_json(source)?;
        let existing_id = existing
            .get("evaluation_id")
            .and_then(Value::as_str)
            .ok_or_else(|| io::Error::other("staged evidence archive has no evaluation_id"))?;
        if existing_id != evaluation_id {
            return Err(io::Error::other(
                "staged evidence archive identity does not match its filename",
            ));
        }
        ...
        if archive_id != evaluation_id {
            return Err(io::Error::other(
                "evidence archive identity changed while staging",
            ));
        }
    ...
    let archive_id = archive
        ...
        .ok_or_else(|| io::Error::other("evidence archive has no evaluation_id"))?;
    if archive_id != evaluation_id {
        return Err(io::Error::other("evidence archive identity mismatch"));
    }
